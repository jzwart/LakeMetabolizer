...
#'@export
metab.mle <- function(do.obs, do.sat, k.gas, z.mix, irr, wtr, error.type="OE", bootstrap=F, n.boot=1000, ar1.resid = T,...){
  ...
  if(any(z.mix <= 0)){
	...
	if(error.type=='OE'){
		...
		if(bootstrap){
		  bootResults<-bootstrap.metab(guesses=guesses,pars=pars0,fn=mleNllOE,do.obs=do.obs, do.sat=do.sat, error.type=error.type, freq=freq,
		                               k.gas=(k.gas/freq), z.mix=z.mix, irr=irr, wtr=wtr, n.boot=n.boot, ar1.resids = ar1.resids)
	...
	if(bootstrap){
	  return(list("params"=pars, "metab"=c("GPP"=GPP,"R"=R,"NEP"=GPP+R,"GPP_SD"=GPP_sd,"R_SD"=R_sd,
	                                       "gppCoeff_SD"=gppCoeff_sd, "rCoeff_SD"=rCoeff_sd, "doInit_SD"=doInit_sd)))
	}else{
